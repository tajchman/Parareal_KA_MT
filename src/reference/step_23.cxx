...
namespace Step23
{
  ...
  {
  public:
    WaveEquation(const char *p = ".", const char *logName = nullptr);
    void run();
  ...
  template <int dim>
  WaveEquation<dim>::WaveEquation(const char *p, const char *logName)
    : fe(1)
    ...
    , outDir(p)
  {
    if (logName)
    {
      f.open(logName, std::ios_base::out);
      log.attach(f);
      log.push("Run  ");
    }
 ...
 #include <iomanip>
  ...
  void WaveEquation<dim>::output_results() const
  {
    ...
    const std::string filename = outDir +
      "/solution-" + Utilities::int_to_string(timestep_number, 6) + ".vtu";
  ...
  void WaveEquation<dim>::run()
  {
    ...
    for (; time <= 0.1; time += time_step, ++timestep_number)
      {
        std::cerr << "Time step " << timestep_number << " at t=" << time
                  << "      \r";
        ...
        log << __FILE__ << "(" << __LINE__ << ") " 
            << std::setprecision(18) << "u " << timestep_number << " " << solution_u << "\n";
        log << __FILE__ << "(" << __LINE__ << ") " 
            << "v " << timestep_number << " " << solution_v << "\n";
  ...
  try
    {
      ...
      WaveEquation<2> wave_equation_solver("out/reference", "reference.log");
